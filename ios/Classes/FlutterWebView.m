...
- (instancetype)initWithFrame:(CGRect)frame
  ...
  if (self = [super init]) {
    ...
    if (@available(iOS 11.0, *)) {
      ...
      NSArray<NSDictionary*>* cookies = args[@"cookies"];
        NSLog(@"%@",cookies);
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if([keyPath isEqualToString:@"title"] ){
        ...
        if(title == nil){ title = @"";}
        [_navigationDelegate onPageChangeTitle: title];
        
    } else if ([keyPath isEqualToString:@"URL"]){
        NSString * url = _webView.URL.absoluteString;
        if(url == nil){ url = @"";}
        [_navigationDelegate onPageJumpURL: url];
    }
}

- (void) setCookie:(NSArray<NSDictionary *> *) cookies{
    NSMutableArray<NSHTTPCookie *> * resultCookies = [NSMutableArray new];
    for(NSDictionary * data in cookies){
        NSString * name = [self safeValue: data forKey:@"name"];
        NSString * value = [self safeValue: data forKey:@"value"];
        NSString * domian = [self safeValue: data forKey:@"domain"];
        NSString * path = [self safeValue: data forKey:@"path"];
        ...
        [resultCookies addObject:cookie];
    }
    ...
    if (@available(iOS 11, *)) {
        ...
        [cookieStore getAllCookies:^(NSArray* cookies) {
            for(NSHTTPCookie * cookie in resultCookies){
                if(cookie.value == nil || cookie.value.length <= 0) {
                    for (NSHTTPCookie *oldCookie in cookies) {
                        if([oldCookie.name isEqualToString:cookie.name]){
                            [cookieStore deleteCookie:oldCookie completionHandler:nil];
                        }
                    }
                }else {
                    [cookieStore setCookie:cookie completionHandler:nil];
...
- (NSString * )safeValue:(NSDictionary *)data forKey:(NSString *)key{
    if(data == nil) return  @"";
...
- (void)onMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  ...
  } else if ([[call method] isEqualToString:@"updateCookies"]){
    [self updateCookie:call result:result];
  }  else {
...
- (void)updateCookie:(FlutterMethodCall*)call result:(FlutterResult)result {
    
    NSDictionary* cookies = [call arguments];
    NSMutableArray<NSHTTPCookie *> * resultCookies = [NSMutableArray new];
    for(NSDictionary * data in cookies){
        NSString * name = [self safeValue: data forKey:@"name"];
        NSString * value = [self safeValue: data forKey:@"value"];
        ...
        [cookieProperties setObject:path forKey:NSHTTPCookiePath];
        NSHTTPCookie *cookie = [NSHTTPCookie cookieWithProperties: cookieProperties];
        NSLog(@"get cookies : -- %@", cookieProperties);
        [resultCookies addObject:cookie];
    }
